...
import it.pagopa.generated.ecommerce.helpdesk.v2.model.DeadLetterSearchDateTimeRangeDto
...
class DeadLetterDataProviderTest {
    ...
    fun `Should find result for all dead letter events without time range`() {
        ...
        val deadLetterEvents =
            listOf(
                HelpdeskTestUtilsV2.buildDeadLetterEvent("queue1", "test1"),
                HelpdeskTestUtilsV2.buildDeadLetterEvent("queue2", "test2")
            )
    ...
    fun `Should find result for specific source dead letter events without time range`(
        ...
        val deadLetterEvents =
            listOf(
                HelpdeskTestUtilsV2.buildDeadLetterEvent(source, "test1"),
                HelpdeskTestUtilsV2.buildDeadLetterEvent(source, "test2")
            )
        val expectedDeadLetterDtoList =
            deadLetterEvents.map {
                DeadLetterEventDto()
