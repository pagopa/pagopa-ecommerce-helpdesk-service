...
class PMPaymentMethodsDataProviderTest {
    ...
    fun `Should find wallets for an user searching by email`() {
        ...
        val searchParam =
            SearchPaymentMethodRequestEmailDto().type("USER_EMAIL").userEmail("test@test.it")
    ...
    @Test
    fun `Should find wallets for an user searching by fiscal code`() {
        val expectedResponse =
            SearchPaymentMethodResponseDto()
                .name("name")
                .fiscalCode("fiscal_code")
    ...
    fun `Should not find wallets for an user searching by unknown email`() {
        ...
        val searchParam =
            SearchPaymentMethodRequestEmailDto().type("USER_EMAIL").userEmail("test@notexist.it")
        StepVerifier.create(pmPaymentMethodDataProvider.findResult(searchParam))
    ...
    @Test
    fun `Should not find wallets for an user searching by unknown fiscalCode`() {
        Hooks.onOperatorDebug()
        val searchParam =
            SearchPaymentMethodRequestFiscalCodeDto()
                .type("USER_FISCAL_CODE")
                .userFiscalCode("unknownFiscalCode")
        StepVerifier.create(pmPaymentMethodDataProvider.findResult(searchParam))
            .expectError(NoResultFoundException::class.java)
            .verify()
    }
